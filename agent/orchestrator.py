...
def diagnosis_node(state: AgentState):
    ...
    Image path : vision LLM → product recommendation (2 calls, unavoidable).
    Text path  : ChromaDB retrieval (no LLM) → single combined LLM call for
                 both diagnosis and recommendation, cutting latency by ~50%.
    """
    img = state.get("image_bytes")
    if img:
        # Vision path — still requires two calls (vision + recommendation).
    ...
    # Text path — single combined LLM call (RAG retrieval is free, no LLM).
    ...
    final_text = llm.invoke(combined_prompt).content
    return {"response_text": final_text}
...
class AgroMindAgent:
    ...
    async def run(
        ...
        if _intent in ("Diagnosis", "Product") and "No suitable product found" not in _raw_text:
            # Run synchronously so the UI has the new tasks immediately for the rerun
            _save_treatment_in_background(session_id, _raw_text, user_text)
